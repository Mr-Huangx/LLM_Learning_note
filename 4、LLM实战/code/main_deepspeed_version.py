'''
前面main.py只是训练的示范代码，但是在实际的训练中:
1、我们是无法通过单卡进行训练的，因为单卡无法存放LLM的所有参数，以及训练过程中的梯度、Adam的动量以及激活值，因此我们使用deepspeed框架完成训练
2、我们需要可能会受到某些不可控影响，模型训练中途中断
3、需要输出对应的日志，方便后期的查看，回溯
综上，我们这里完成一个完整的训练脚本，之后使用bash在linux中基于deepspeed框架进行训练
'''
...
def main():
    ...
    # 加载脚本参数
    print("evaluation_strategy" in TrainingArguments.__dataclass_fields__)
    print("eval_strategy" in TrainingArguments.__dataclass_fields__)
